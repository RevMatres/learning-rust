 ...
 *
 */
pub fn go(closure: Fn) {
    let opengl = OpenGL::V3_2;

    let window = Window::new("TEEEETRIS!", [100, 100], opengl);

    let piston_render = Engine::new(window, closure)
                            .handle_events();
}
...
impl Engine {
    ...
    pub fn handle_events(&mut self) {
        while let Some(event) = &mut self.event_qeue.next(&mut self.glutin_window) {
            ...
            /*
             *
             * here goes the rendering and event handling code
             *
             *
             *
             *
             */
            /*f let Some(render_args) = event.render_args() {
                use self::graphics::*;
